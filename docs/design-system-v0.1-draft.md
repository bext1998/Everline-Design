...
Tooltip（issue #39，issue #17 15 群清單第 13 項，2026-08-05）：`works/html/batch1/` 的 HTML/CSS candidate **2026-08-14 人類 G1 視覺審查通過，已畢業，仍是 candidate 未宣告 stable**；master.svg 對應的 `c-tooltip` 群組已標記 `superseded-by: works/html/batch1/index.html`，HTML/CSS 是這一群目前的候選規格權威來源。純 CSS `:hover`／`:focus-within`、token 核對與兩種固定 placement 的限制維持不變，詳見 `tokens/everline-draft.tokens.json` 與 `docs/STATUS.md`。issue #17 15 群清單中，已畢業 5 群＋等待 G1 的 5 群共 10 群，仍有 5 群未動工。
...
狀態：`candidate`（2026-08-14，issue #39：人類 G1 視覺審查通過，已畢業；HTML/CSS 是候選規格權威來源，未宣告 stable）
