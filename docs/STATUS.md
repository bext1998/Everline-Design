...
- 2026-07-28：Breadcrumb（issue #7）SVG 候選稿經一輪人工審查修正——原始靠左排列改為每項文字置中於自己的命中區（以 Chrome `getBBox()` 實測文字寬度，非目測），並修正 Loading／Search field 兩區塊間距過窄（原僅 18px，覆蓋到下一區塊標題）的既有排版問題。畢業到 HTML/CSS 時，`<li>` 使用 `display: contents` 讓子元素直接參與 flex 版面，命中區置中改由原生 padding 自然達成，不需複製 SVG 手算的 slot 座標；過程中發現並修正一個真實 bug：`display: contents` 在同優先權下會蓋掉瀏覽器預設的 `[hidden]{display:none}`，導致 collapsed 範例的摺疊項目一開始就沒被正確隱藏（與本檔案既有 `.loading-overlay[hidden]` 註解記載的同一類陷阱）。Number input（issue #8）延伸 Text input／Icon button 既有 token，畢業時改用原生 `<input type="number">` 搭配 `stepUp()`/`stepDown()`，min/max 到界時對應按鈕即時原生 `disabled`（點擊或直接輸入到界皆會觸發）。兩者的像素級驗證（computed style 讀值＋螢幕截圖 RGB 採樣）：number-input 欄位背景 `rgb(102,102,102)`＝`#666666`、focus 邊框實測 `rgb(89,138,232)`＝`#598AE8`；breadcrumb 預設文字色 `rgb(77,77,77)`＝`#4d4d4d`、目前頁／hover 文字色 `rgb(242,242,242)`＝`#f2f2f2`，皆與 token 精確吻合。
