...
#### 1.2b HTML/CSS 候選元件原型管理契約（2026-07-19 新增，同日修正流程順序）

- 輸入要求：新增或調整 `works/html/` 下的候選元件時，必須先在 Illustrator 完成構思草稿，再以 SVG 產生候選稿供人類視覺審查並確認沒有問題，才能轉為 HTML/CSS；不得跳過 SVG 候選審查階段直接以 HTML/CSS 宣告候選規格。
- 輸出承諾：候選稿的視覺數值須對應 `tokens/` 既有語意 token，不得另建一套命名；圖示仍依既有圖示來源規則以 SVG 呈現，不併入 HTML/CSS 元件外殼；技術棧未驗證前維持框架中立的原生 HTML/CSS/JS。元件轉為 HTML/CSS 後即「畢業」，該元件之後以 HTML/CSS 版本為權威來源，對應 SVG 版本標記為歷史快照、不再更新。
- 失敗行為：若元件尚未經 SVG 候選審查確認，不可直接以 HTML/CSS 宣告為候選規格；不得因採用 HTML/CSS 就預設特定前端框架或視為已進入前端元件庫實作。
